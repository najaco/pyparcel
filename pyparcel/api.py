...
pack_dict = {
    ...
    str: (lambda obj: struct.pack("q{}s".format(len(obj)), len(obj), obj.encode(ENCODING))),
...
unpack_dict = {
    int: (
        lambda _, data: (struct.unpack("i", data[: size_dict[int]])[0], data[size_dict[int]:])
    ),
    bool: (
        lambda _, data: (struct.unpack("?", data[: size_dict[bool]])[0], data[size_dict[bool]:])
    ),
    float: (
        lambda _, data: (
